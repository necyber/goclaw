...
### 4.1 主程序修改
- [x] 修改 `cmd/goclaw/main.go`
- [x] 在配置加载后初始化 HTTP 服务器
- [x] 创建处理器实例
- [x] 启动 HTTP 服务器（在单独的 goroutine）
- [x] 修改信号处理，同时关闭引擎和 HTTP 服务器
- [x] 确保优雅关闭顺序正确
...
### 6.1 端到端测试
- [x] 创建 `pkg/api/integration_test.go`
- [x] 实现测试服务器启动辅助函数
- [x] 测试完整的工作流生命周期
  - [x] 提交工作流
  - [x] 查询工作流状态
...
### 6.3 性能测试
- [x] 使用 `go test -bench` 进行基准测试
- [x] 测试健康检查响应时间
- [x] 测试工作流提交响应时间
- [x] 测试状态查询响应时间
- [x] 创建 `pkg/api/benchmark_test.go`
...
### 7.3 示例和教程
- [x] 创建基本使用示例（已在 README 中）
- [x] 创建 API 使用示例（docs/examples/curl-examples.md）
- [ ] 创建高级用例示例（可选）
- [ ] 编写故障排查指南（可选）
