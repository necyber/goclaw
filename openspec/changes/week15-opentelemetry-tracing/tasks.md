...
- [ ] 6.1 实现 exporter 不可用时的错误隔离策略（不影响业务请求）
- [ ] 6.2 对 tracing 失败路径增加告警级日志与调试上下文字段
- [ ] 6.3 增加 exporter 故障注入测试，验证请求路径可用性
- [ ] 6.4 增加关闭阶段 flush 超时测试，验证进程可有界退出
...
- [ ] 8.1 新增/更新 OpenTelemetry tracing 使用指南文档
- [ ] 8.2 更新配置说明文档（字段含义、默认值、迁移说明）
- [ ] 8.3 更新运行手册（collector 连接、故障排查、关闭语义）
- [ ] 8.4 在 README 或架构文档补充 tracing 架构图/流程说明
